...
class HomePage(BasePage):
    ...
    login_button = (By.CSS_SELECTOR, "button[ng-show=\"custId != ''\"]")
    ...
    def click_customer_btn(self):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.customer_button)).click()

    def click_bank_btn(self):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.bank_button)).click()

    def select_customer_name(self, customer_name: str):
        dropdown = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.user_select))
